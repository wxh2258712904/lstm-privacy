...
#define hidenode  4    //隐藏结点数，存储“携带位”
...
public:
    ...
    double W_G[innode][hidenode];     //用于输入与隐含层产生新记忆的权值矩阵
    double U_G[hidenode][hidenode];   //用于隐含层之间产生新记忆的权值矩阵
...
RNN::RNN()
{
    x = new double[innode];//x是输入向量 
    y = new double[outnode];//y是输出向量
    winit((double*)W_I, innode * hidenode);//连接输入与隐含层单元中输入门的权值矩阵
    winit((double*)U_I, hidenode * hidenode);//连接上一隐层输出与本隐含层单元中输入门的权值矩阵
    winit((double*)W_F, innode * hidenode);//连接输入与隐含层单元中遗忘门的权值矩阵
    winit((double*)U_F, hidenode * hidenode);//连接上一隐含层与本隐含层单元中遗忘门的权值矩阵
    winit((double*)W_O, innode * hidenode);//连接输入与隐含层单元中遗忘门的权值矩阵
    winit((double*)U_O, hidenode * hidenode);//连接上一隐含层与现在时刻的隐含层的权值矩阵
    winit((double*)W_G, innode * hidenode); //用于输入与隐含层产生新记忆的权值矩阵
    winit((double*)U_G, hidenode * hidenode);//用于隐含层之间产生新记忆的权值矩阵
    winit((double*)W_out, hidenode * outnode);//连接隐层与输出层的权值矩阵
}
...
void RNN::train()
{
    ...
    for (epoch = 0; epoch < 11000; epoch++)  //训练次数
    {
        ...
        memset(predict, 0, sizeof(predict));   //将predict归零
